...
p_0 = 0.076275 #p, radius radius in units stellar radii
b_0 = .143 #impact parameter
rho_s_0 = 1150 #set stellar density, in SI
q1_0 = .4761 #quadratic limb darkening coefficient 1
q2_0 = .34058 #quadratic limb darkening coefficient 2
...
def load_data(show=True): #function to input lightcurves, load curves dictionary, and precompute info
    ...
    tmids_df_path = Path("/Users/johnnymoynihan/Downloads/kepler9_holczer_inner.csv")
    ...
    transit_duration = 0.15927
    period = 19.2259
    halfperiod = period / 2
    cushion = 3 * transit_duration


    data = [] #create list of inner planet data 
    for i in range(len(tmids)):
        ...
        if not np.isnan(lc_i).all():
            ...
            plt.plot(lc_i, fl_i, '.')
            plt.axvline(tmids[i], color="red", linestyle="--", linewidth=1.5, label=f"tmidi_{i}")
            plt.xlabel("Time")
            plt.ylabel("Flux")
            plt.title(f"Transit {i}")
            plt.show()
            data.append((f"transit{i}", tmids[i], tmids_lineph[i], time_bounds_i, lc_i, fl_i, ferr_i))
    ...
    for transit_name, tmid_obs, tmid_lineph, time_bounds_i, t, flux, ferr in data: #loop over each lightcurve
...
def create_info(data_info, curves): #create info dictionary for algorithm
    ...
    #set mcmc info
    mcmcpoints = 75000 #set number of points to test
    thin = 10 #set thin
...
def main():
    ...
    #save results
    with open("mcmc_curves_kepler9b.pkl", "wb") as f:
        pickle.dump({
            "info": info,
            "curves" : info["curves"],
